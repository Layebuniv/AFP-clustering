...
• 7 seed selection strategies (6 classiques + gated)
• True shortest-path arrival propagation
• Optional post-propagation noise/straggler resolution

Author: Abdeslem LAYEB
License: MIT
"""
...
class ArrivalTimeFrontPropagation:
    ...
    def fit(self, X):
        ...
        seeds = self._select_seeds(X, rho, speed, knn_idx)
...
if __name__ == "__main__":
    ...
    # Classic speed-farthest
    base = ArrivalTimeFrontPropagation(
        n_clusters=4,
        seed_strategy="speed_farthest",
        resolve_unlabeled=False
    ).fit(X)

    # Gated seeding + noise resolution
    resolved = ArrivalTimeFrontPropagation(
        n_clusters=4,
        seed_strategy="gated",
    ...
    # Gated seeding only
    gated_only = ArrivalTimeFrontPropagation(
        n_clusters=4,
        seed_strategy="gated",
        resolve_unlabeled=False
    ).fit(X)
